...
try:
        ...
        if "recup_n.sh" in path_script :
            ...
            if not  re.match(r'^[0-9]+$', nb_lignes):
                ...
                for fichier_tsv in fichiers_tsv :
                    ...
                    tac = time()
                    conversion_times.append(tac-tic)
    ...
    time_convert = sum(conversion_times)
    if time_convert < 60 :
        s = ""
        if len(fichiers_tsv) > 1 :
            s="s"
        print(f"OK convert in {round(time_convert, 2)}s  ({len(fichiers_tsv)} file{s}).")

    elif time_convert >= 60 and time_convert < 3600 :
