...
class rapoFilm:
    ...
    def communNode(self) -> list:
        ...
        Exemple :
        ```
        common_nodes = rapoFilm.communNode()
        ```
    ...
    def communBranch(self) -> dict:
        ...
        Exemple :
        ```
        common_branches = rapoFilm.communBranch()
        ```
    ...
    def threadPathBegin(self, n : int, nodeValeur: list) -> None:
        ...
        Exemple :
        ```
        rapoFilm.threadPathBegin(5, [], ["Node1", "Node2"])
        ```
    ...
    def threadPathEnd(self, j : int, nodeValeur: list) -> None:
        ...
        Exemple :
        ```
        rapoFilm.threadPathEnd(5, [], ["Node1", "Node2"])
        ```
    ...
    def path(self, result={}, key=[]) -> list:
        ...
        Exemple :
        ```
        result_path = rapoFilm.path( result={"debut": {"A": [1, 2]}, "fin": {"B": [2, 3]}, "commun": [2]}, key=[])
        result_path = rapoFilm.path( result={}, key=["A"])
        ```
    ...
    def threadDebut(self) -> None:
        """
        Thread 1 soit P1 crée le graph à partir du tconstDebut et envoie le résultat à threadPasserelle (P3)

        Exemple : 
        ```
        resultat = {"type": "fin", noeud1 : ['Film1','Film2], noeud2 : ['Film']}
        ```
        """
    ...
    def threadFin(self) -> None:
        """
        Thread 2 soit P2 crée le graph à partir du tconstFin et envoie le résultat à threadPasserelle (P3)
    ...
    def threadPasserelle(self) -> None:
        """
        Thread 3 P3 maitre d'oeuvre qui teste s'il existe une connection entre tconstDebut et tconstFin
    ...
    def Start(self) -> dict:
        """
        Methode mere de la classe rapoFilm retourne le cheminF entre les 2 films.
    
        :return: Dictionnaire contenant le message et les données.

        Exemple :
        ```
        result = rapoFilm.Start()
        ```
        """
        dic_f = {}
